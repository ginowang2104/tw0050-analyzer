...
FALLBACK_0050 = {
    ...
    "2881": "富邦金", "2882": "國泰金", "2303": "聯電", "3711": "日月光投控", "2891": "中信金",
    "2412": "中華電", "2886": "兆豐金", "2884": "玉山金", "5880": "合庫金", "1216": "統一",
    "2892": "第一金", "2880": "華南金", "2357": "華碩", "3008": "大立光", "2379": "瑞昱",
    "2885": "元大金", "2327": "國巨", "2887": "台新金", "2890": "永豐金", "1301": "台塑",
    "1303": "南亞", "3034": "聯詠", "2376": "技嘉", "6669": "緯穎", "2395": "研華",
    "2883": "開發金", "2319": "鴻準", "1101": "台泥", "4904": "遠傳", "3045": "台灣大",
    "1326": "台化", "2888": "新光金", "2408": "南亞科", "5871": "中租-KY", "6505": "台塑化",
    "2603": "長榮", "2609": "陽明", "2615": "萬海", "3231": "緯創", "2353": "宏碁",
    "2344": "華邦電", "2409": "友達", "3481": "群創", "2301": "光寶科", "3037": "欣興"
}
...
def fetch_0050() -> dict[str, str]:
    ...
    try:
        ...
        r = requests.get(url, headers=wg_headers, timeout=25, verify=False)
        r.encoding = "utf-8"
        
        if r.status_code == 200:
            # 使用更強韌的正則表達式，處理可能的換行或空格
            # 匹配樣式: <a href="/stock/2330">2330</a></td><td>台積電</td>
            matches = re.findall(r'href="/stock/(\d+)".*?>(\d+)</a>\s*</td>\s*<td>\s*([^<]+)</td>', r.text)
            res = {m[1]: m[2].strip() for m in matches if m[0] == m[1]}
            
            if len(res) >= 45:
                print(f"  → [成功] 玩股網 (取得 {len(res)} 檔)")
                return res
        print(f"  [跳過] 玩股網回傳狀態碼: {r.status_code}")
    except Exception as e:
        print(f"  [跳過] 玩股網異常: {e}")

    # 策略 B：證交所官方 API (TWT38U) - 第二備援
    try:
        # 使用今天日期抓取
        target_date = datetime.now().strftime("%Y%m%d")
        twse_url = f"https://www.twse.com.tw/fund/TWT38U?response=json&stockNo=0050&date={target_date}"
        r = requests.get(twse_url, headers=HEADERS, timeout=20, verify=False)
        data = r.json()
        
        if data.get("stat") == "OK" and "data" in data:
            # row[1] 是代號, row[2] 是名稱 (證交所 API 索引可能略有不同，需過濾)
            res = {}
            ...
            if len(res) >= 45:
                print(f"  → [成功] 證交所 API (取得 {len(res)} 檔)")
                return res
    except Exception as e:
        print(f"  [跳過] 證交所 API 異常: {e}")

    # 策略 D：Yahoo Finance (當台灣網站全滅時的最後希望)
    try:
        y_url = "https://query1.finance.yahoo.com/v7/finance/quote?symbols=0050.TW"
        # 注意：Yahoo 可能需要特定的 crumbs 或更複雜的 headers
        r = requests.get(y_url, headers=HEADERS, timeout=20)
        # Yahoo 的 0050 頁面較難直接抓到成份股清單，通常需要配合 yfinance 套件
        # 建議目前先以強健的 FALLBACK_0050 為主
    except:
        pass

    # 策略 C：硬編碼最後防線 (確保分析邏輯不崩潰)
    print("  → [警告] 線上來源全數失敗，使用硬編碼備援名單")
    # 確保回傳一個非空的字典，避免 analyze 判定成 0 檔導致「42 檔加入」的錯誤
